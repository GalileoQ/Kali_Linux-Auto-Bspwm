...
if [ "$user" == "root" ]; then
	...
	sleep 2
	sudo apt install -y kitty rofi feh xclip ranger i3lock-fancy scrot scrub wmname imagemagick cmatrix htop btop fastfetch python3-pip procps tty-clock fzf lsd bat pamixer flameshot glances 
	if [ $? != 0 ] && [ $? != 130 ]; then
